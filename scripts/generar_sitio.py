...
function verPolla(ref) {{
  ...
  if (score && REALES && REALES.ronda_16avos) {{
    ...
    html += '<div class="mc-header"><span>Tu predicción</span><span>Slot</span><span>Equipo real</span><span>Slot real</span><span>Pts</span></div>';
    ...
    (polla.ronda_16avos||[]).forEach(e => {{
      ...
      if (eq && sr) {{
        pts = (sr === sp) ? 2 : 1;
        cls = (sr === sp) ? 'mc-hit' : 'mc-miss';
      }}
      
      html += '<div class="mc-row '+cls+'"><span class="mc-team">'+(eq||'—')+'</span><span class="mc-pred">'+sp+'</span><span class="mc-real">'+(er||'—')+'</span><span class="mc-slot">'+(sr||'—')+'</span><span class="mc-pts">'+(pts?'+'+pts:'')+'</span></div>';
    }});
    html += '</div>';
  ...
  rondasPred.forEach(([title, entries, showSlot, total]) => {{
    ...
    for (let i = 0; i < total; i += 2) {{
      ...
      html += '<div class="mbm-teams"><span class="mbm-pred">'+(e1.equipo||'—')+'</span><span class="mbm-vs">vs</span><span class="mbm-pred">'+(e2.equipo||'—')+'</span></div>';
    }}
    html += '</div>';
  }});
  html += '</div>';
